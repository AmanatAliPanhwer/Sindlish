...
class TestReassignment:
    def test_reassign_same_type(self):
        interp, _ = run("x = 1\nx = 2")
        assert interp.variables["x"]["value"] == 2
...
class TestDynamicTyping:
    def test_int_to_string(self):
        interp, _ = run('x = 42\nx = "text"')
        assert interp.variables["x"]["value"] == "text"
    ...
    def test_int_to_float(self):
        interp, _ = run("x = 5\nx = 5.5")
        assert interp.variables["x"]["value"] == 5.5


class TestMultipleVariables:
    def test_multiple_variables(self):
        interp, _ = run("a = 1\nb = 2\nc = 3")
        assert interp.variables["a"]["value"] == 1
        assert interp.variables["b"]["value"] == 2
        assert interp.variables["c"]["value"] == 3

    def test_variable_in_expression(self):
        interp, _ = run("a = 5\nb = a + 10")
        assert interp.variables["b"]["value"] == 15

    def test_self_referencing_reassignment(self):
        interp, _ = run("x = 5\nx = x + 1")
        assert interp.variables["x"]["value"] == 6
