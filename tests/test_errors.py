...
class TestConstReassignment:
    def test_const_reassignment_raises(self):
        with pytest.raises(RuntimeError, match="pakko"):
            run("pakko adad x = 10\nx = 20")


class TestTypeMismatch:
    def test_adad_rejects_string(self):
        with pytest.raises(TypeError, match="adad"):
            run('adad x = "hello"')

    def test_lafz_rejects_int(self):
        with pytest.raises(TypeError, match="lafz"):
            run("lafz x = 42")

    def test_dahai_rejects_int(self):
        with pytest.raises(TypeError, match="dehai"):
            run("dahai x = 42")
