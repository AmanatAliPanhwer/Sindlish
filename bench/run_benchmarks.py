...
def benchmark_thread():
    ...
    subprocess.run(
        ["cargo", "build", "--release"], cwd="bench/rust", capture_output=True
    )
    ...
    for test in TESTS:
        for lang in LANGS:
            ...
            if lang == "Sindlish":
                ...
                cmd = [
                    os.path.join(
                        "bench", "rust", "target", "release", "sindlish-bench"
                    ),
                    test["type"],
                ]
                if test["arg"]:
...
def make_layout():
    ...
    layout["main"].split_row(
        Layout(name="code", ratio=1), Layout(name="results", ratio=1)
    )
    return layout
...
def update_dashboard(layout):
    # Header
    status = (
        f"[bold green]Running {current_running['lang']}: {current_running['test']}"
        if current_running["lang"] != "DONE"
        else "[bold blue]BENCHMARKS COMPLETE"
    )
    layout["header"].update(
        Panel(status, title="Sindlish vs Python vs Rust Performance")
    )

    # Code View
    layout["code"].update(
        Panel(current_running["code"], title=f"Source Code: {current_running['lang']}")
    )
    ...
    layout["footer"].update(
        Panel("[dim]Algorithmic Parity Guaranteed | Naive Recursion | No JIT Hacks[/]")
    )
