...
The resolver (`interpreter/analysis/resolver.py`, ~460 lines) answers all of it in one walk, **before any code runs**. It's the interpreter's accountant: every name gets audited, every variable gets a numbered mailbox (a **slot**), and dishonest type annotations get flagged while correction is still cheap.
...
Dispatch is by naming convention (`resolver.py:67`): visiting an `IfNode` calls `resolve_IfNode`; nodes without a dedicated method get a generic visitor that recurses into every attribute.
...
Follow the clerk's hands through `resolve_FunctionNode` (`resolver.py:384`):
...
- **Function names don't occupy slots.** `define_function` (`resolver.py:230`) just registers the name; references compile to `LOAD_GLOBAL`. This came out of bug-fix history — the older scheme reserved slots for function values and broke top-level reads.
...
### Walkthrough 2 — blocks push scopes… mostly

Every block body resolves inside a fresh scope (`resolve_BlockNode`). So you'd expect this to shadow:
...
There is no second `x`. Assignment resolution consults `_find(name)` first (`resolver.py:252`), which searches *all* enclosing scopes — so the inner `x = 5` simply **reuses slot 0** rather than declaring a fresh variable. Blocks create scopes mainly for *reads* like loop iterators; writes collapse onto the nearest existing binding.

> ⚠️ **Known sharp edge (bug logged):** combine that slot-reuse with explicit types and things bite. In a function, after `adad x = 1`, writing `lafz x = "hi"` in an inner block overwrites slot 0's type metadata — and then the *earlier* line fails at runtime complaining it found `ADAD` where `lafz` was expected. Full repro and fix direction live in `roadmap/TODO.md`. Until fixed: don't redeclare annotated variables across block boundaries.

### Walkthrough 3 — loops own their iterator

```sd
har i mein range(3) {
    likh(i)
}
likh(i)      # ← does NOT compile-and-run happily
```
...
1
2
NaleJeGhalti: Nalo 'i' na milyo. Cha tawaan sahi likhyo aahe?
```

`resolve_ForNode` (`resolver.py:328`) pushes a scope just for `i`, allocates its slot, resolves the body, pops. After the pop, `i` is unknown — the loop variable belongs to the loop. (At parse time nothing complained; the failure surfaces at runtime when `LOAD_GLOBAL 'i'` finds nothing. Name errors for never-defined names are a *runtime* event in Sindlish, not a resolve-time one.)
...
Why does `count` survive between calls? Because it doesn't live in a slot anymore. When the resolver sees the inner function **read** `count` (`resolve_VariableNode`, `resolver.py:294`), it notices the owner is an *enclosing* `_FnRec` and registers a capture:
...
Captured locals are compiled as **cells** — little mutable boxes shared by reference (`interpreter/objects/core.py:56`). Slots die with their frame; cells die only when nobody references them. That's why `counter()` keeps ticking after `banayo()` has returned.
...
`resolve_GlobalNode` adds `tally` to `declared_globals`, and assignment resolution checks that set (`resolver.py:153`) before anything else — routing the store to `scope_level = 1`. Without the declaration, `tally = …` inside the function would have created a brand-new *local* slot instead (shadowing the global), which is exactly the classic foot-gun `aalmi` exists to prevent.

### Program-level variables are globals

Anything assigned at the top level gets `scope_level = 1`, `slot_index = -1` (`resolver.py:159`) — they live in the globals `Environment`, not in any frame. That single rule is why this just works:
...
…and why the REPL can keep variables alive across lines (`is_repl` forces the same treatment; `resolver.py:231` region).

### Where type checking plugs in

When an `AssignNode` carries an explicit annotation, `resolve_AssignNode` calls `_verify_assignment_types` (`resolver.py:104`) *before* allocating anything: infer the literal's type, compare, raise `QisamJeGhalti` on mismatch — including per-element checks for `fehrist[adad] = ["a", "b"]`-style lies. That machinery is big enough to deserve its own part: see [typing-model.md](typing-model.md) next chapter.
...
<p>Block writes reuse the nearest binding — true shadowing doesn't happen (and typed redeclaration across blocks is a known bug).</p>
<p>Loop iterators are scoped to the loop.</p>
<p>Closures: reads capture automatically into shared Cells; writes require <code>bahari</code>; captured locals outlive their frame.</p>
